...
class Board {
    ...
    Board(Board board0) {
        ...
        for (int x = 0; x < board0.redPieces(); x++) {
            incrPieces(RED, 1);
        }
        for (int y = 0; y < board0.bluePieces(); y++) {
            incrPieces(BLUE, 1);
        }
        _undoSquares = new Stack<Integer>();
        _undoPieces = new Stack<PieceColor>();
    ...
    boolean legalMove(Move move) {
        ...
        int horizontal = Math.abs(move.col0() - move.col1());
        int vertical = Math.abs(move.row0() - move.row1());
        ...
        for (char a: cols) {
            for (char b: rows) {
                if (get(a, b) == who) {
                    for (int x = -2; x <= 2; x++) {
                        for (int y = -2; y <= 2; y++) {
                            if (get(neighbor(index(a, b), x, y)) == EMPTY) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        if (move.isJump()) {
            ...
            for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                    if (get(neighbor(move.toIndex(), x, y))
                            == _whoseMove.opposite()) {
                        set(neighbor(move.toIndex(), x, y), _whoseMove);
    ...
    void undo() {
        ...
        while (_undoSquares.peek() != null) {
            int prev = _undoSquares.pop();
            PieceColor prevColor = _undoPieces.pop();
            PieceColor currentColor = get(prev);
            incrPieces(prevColor, 1);
            incrPieces(currentColor, -1);
            unrecordedSet(prev, prevColor);

        }
        _undoSquares.pop();
        _undoPieces.pop();
        if (allMoves().get(allMoves().size() - 1).isJump()) {
            _numJumps--;
        }
        _numMoves -= 1;
        _whoseMove = _whoseMove.opposite();
    ...
    boolean legalBlock(char c, char r) {
        ...
        PieceColor b = get(c, r);
        PieceColor reflected1 = get(cols[cols.length
                - 1 - indexc], r);
        PieceColor reflected2 = get(c, rows[rows.length
                - 1 - indexr]);
        PieceColor reflected3 = get(cols[cols.length
                - 1 - indexc], rows[rows.length - 1 - indexr]);
        if (b == EMPTY && reflected1 == EMPTY && reflected2
                == EMPTY && reflected3 == EMPTY) {
            return true;
        }
        return false;
    }
    ...
    void setBlock(char c, char r) {
        ...
        startUndo();
        set(c, r, BLOCKED);
        int a = cols[cols.length - 1 - indexc];
        int b = rows[rows.length - 1 - indexr];
        set(cols[cols.length - 1 - indexc], r, BLOCKED);
        set(c, rows[rows.length - 1 - indexr], BLOCKED);
        set(cols[cols.length - 1 - indexc],
                rows[rows.length - 1 - indexr], BLOCKED);
        if (!canMove(RED) && !canMove(BLUE)) {
            _winner = EMPTY;
        }
